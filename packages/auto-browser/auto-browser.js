...
function AutoBrowser( sequence, options ){
    var self = this;
     ...
     */
    options = options || {};
    this.errorFunc = options.onError || logger.error;
    this.browserShouldPersist = false;
    ...
    //             else this.errorFunc("AutoBrowser@"+funcName+" cannot execute command before first visiting a url. No url visited");
    //         });
    //     }
    // });
}
...
AutoBrowser.prototype = {
    ...
    _flow: function() {
        ...
        if (this.sequence.url != null) this.sequence.river.unshift({
    ...
    run: function() {
        logger.log("AutoBrowser@run ", this.sequence.river)
        commands.push({
            "quit": true
        });
        this.runNextCommand();
    ...
    runNextCommand: function() {
        ...
        if (this.currentInd < this.sequence.river.length)
            this.delegateCommand(this.sequence.river[this.currentInd]);
        this.currentInd++;
     ...
     * AutoBrowser: Starts a fork in the super-river, mainly for organization.
     *
     * SYNTAX:
     * type: COMMAND_TYPE
     * COMMAND_TYPE: {
    ...
    submit: function(form) {
        ...
        var self = this;
        if (!form.button) this.errorFunc("AutoBrowser@_submit ERROR: no login button selector passed for login" + form);
    ...
    collectNextAndBuild: function(result, array, cb, webdriverCollector){
        ...
        if (!cb) this.errorFunc("AutoBrowser@collectNextAndBuild no callback passed");
        logger.log("AutoBrowser@collectNextAndBuild passed: ", arguments);
            ...
            .then(function(elements){
                ...
                function handler(elem, elems){
                    ...
                    .then(function(innerHtml){
                        ...
                        logger.log("AutoBrowser@collect innerHtmls: ", innerHtmls);
                        /**
